...
locals {
  sanitized_name   = replace(replace(replace(replace(lower(var.name), ".modules.", "-m-"), ".externals.", "-m-"), "/[^a-z\\-0-9]/", "-"), "/-*$/", "") #https://github.com/edgelaboratories/terraform-short-name/blob/main/main.tf
